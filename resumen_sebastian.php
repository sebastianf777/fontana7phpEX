		...
		<div class="content">
			<div class="container">
				<div class='wrap-table100'>
					...
					<div class="logoB">
						     <div class="divLogo">
								<h1 class="h1Logo">VENTAS MOSTRADOR - Sebastian </h1>
						     </div>
						     </div>
						...
						<table class="tabla_izq">
							...
							<tbody>
								<?php
								$sql = "SELECT SUM(detallepedido) AS totalpedidos, fechaing from productos WHERE detallepedido != '' AND serie = 'Sebastian' GROUP BY fechaing order by id DESC ";
								$result = mysqli_query($con, $sql);
								while ($crow = mysqli_fetch_assoc($result)) {
								?>
									<tr class='row100'>
							...
							<tbody>
								...
								FROM productos WHERE pagotipo = 'debito' AND serie = 'Sebastian' order by id DESC ";
								$result = mysqli_query($con, $sql);
								while ($crow = mysqli_fetch_assoc($result)) {
								?>
									<tr class='row100'>
							...
							<tbody>
								...
								$sql = "SELECT detallemateriales, producto, ferreteria, detalleferreteria, id, pagotipo, acuenta, cliente, fechaing, pagotipo
								 FROM productos WHERE serie = 'Sebastian'and (cliente != '' OR pagotipo = 'fiado') order by id DESC ";
								$result = mysqli_query($con, $sql);
								while ($crow = mysqli_fetch_assoc($result)) {
								?>
									<tr class='row100'>
						...
						<table class="resumenes" data-vertable='ver1'>
							...
							<thead id="tabla">
								<tr class='row100 head'>
									...
									<th class='ventas-totales-diarias-2b' data-column='column2'>
										<center class="producto">TOTAL MATERIALES, FERRETERIA, Y PEDIDOS - Sebastian</center>
									</th>
							...
							<tbody>
								...
								?>
									<tr class='row100'>
									    ...
									    <td>
											<center class="fecha_resumenes"><?php echo $crow['fechaing']; ?></center>
										</td>
